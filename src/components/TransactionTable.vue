      ...
      <template v-slot:default>
        ...
        <transition-group name="list-complete" tag="tbody" v-else>
          <tr v-for="tx in transactions" :key="tx.id" class="row-item">
            ...
            <td>
              <v-btn label small :to="{name: 'transaction', params: {id: tx.id}}" style="text-transform: none;height:26px;font-weight:500;letter-spacing: 0;color:#EBB077 !important;background-color: #FFFAF8 !important;font-weight: 600;padding:0 12px !important;" color="point" rounded outlined class="darken-1 px-2 mx-1" :target="embedMode ? '_blank' : ''" v-if="tx.firstActionTypeId">
                <v-btn icon x-small color="point" :to="{query: {action: tx.firstActionTypeId}, ...((detail || embedMode) ? {} : {name: 'transactions'})}" style="width:12px;" class="mr-1"><v-icon x-small>mdi-filter-variant</v-icon></v-btn>
                {{tx.firstActionTypeId}}
              </v-btn>
            </td>
            <td v-if="detail">
            ...
            <td v-if="detail || embedMode">
              <router-link :to="{name: 'avatar', params: {address: tx.firstAvatarAddressInActionArguments}}" v-if="tx.firstAvatarAddressInActionArguments" class="mx-1" :target="embedMode ? '_blank' : ''">
                {{tx.firstAvatarAddressInActionArguments.substr(0, 8)}}
              </router-link>
            </td>
          </tr>
        </transition-group>
