  ...
  <v-menu offset-y v-model="onoff">
    <template v-slot:activator="{on,attrs}">
      <v-btn class="menu-down-btn" outlined :color="color" rounded v-bind="attrs" v-on="on" @click="open">
        {{ items.find(item => item.value === value)?.label || value || 'All Items'}}
        <v-icon :color="color" class="ml-1" small @click.stop.prevent="clearItem" v-if="value">mdi-close</v-icon>
        <v-icon :color="color" class="ml-1" small>mdi-chevron-down</v-icon>
      </v-btn>
    </template>
    ...
    <div style="background-color: white">
      ...
      <v-list class="menu-down-list">
        ...
        <v-list-item @click="select({value: search})" v-if="search > 1000">
          <v-list-item-title>item id : {{ search }}</v-list-item-title>
        </v-list-item>
      </v-list>
    </div>
  </v-menu>
...
export default {
    ...
    async created() {
        this.items = [
          ...
          {value: 'appotion', label: 'AP Potion'},
        ];
    },
    methods: {
        open() {
